    ...
    <div class="form-group">
        @for($i = 1; $i <= 7; $i++)
            <div class="day-container">
                <div class="checkbox">
                    <label>
                        <input type="checkbox" class="offer-day-switcher-blind{{ $i >= 1 && $i <= 5 ? ' offer-day-work' : '' }}{{ $i >= 6 && $i <= 7 ? ' offer-day-weekends' : '' }}" name="days[{{ $i }}][enabled]" data-day="{{ $i }}" id="day_{{ $i }}" value="1" {{ Input::old('days.'.$i.'.enabled', isset($days[$i]) ? 1 : 0) == 1 ? 'checked' : '' }}>
                        {{ trans('global.days.'.$i) }}
                    </label>
                </div>
                <div class="form-inline">
