                    <form role="form" method="POST" action="{{ $offer->id ? URL::route('requestPost', ['id' => $offer->id]) : URL::route('requestPost') }}" id="edit-request-form">
                        ...
                        <div class="form-group {{ $errors->has('date') ? 'has-error' : '' }}">
                            ...
                            <div class="form-inline">
                                {{ Form::selectDays('date[day]', Input::old('date.day', Date::parse($offer->date)->format('j')), ['class' => 'form-control']) }}
                                {{ Form::selectMonths('date[month]', Input::old('date.month', Date::parse($offer->date)->format('n')), ['class' => 'form-control']) }}
                                {{ Form::selectYears('date[year]', null, Input::old('date.year', Date::parse($offer->date)->format('Y')), ['class' => 'form-control']) }}
                            </div>
                        </div>
                        <div class="form-group {{ $errors->has('time') ? 'has-error' : '' }}">
                            ...
                            <div class="form-inline">
                                <select class="form-control" name="time[hours]" id="time_hours">
                                    @for($j=0;$j<=23;$j++)
                                        <option value="{{ $j }}"{{ Input::old('time.hours', Date::parse($offer->time)->format('G')) == $j ? ' selected' : '' }}>{{ strlen($j) == 1 ? '0'.$j : $j }}</option>
                                    @endfor
                                </select>
                                <select class="form-control" name="time[minutes]" id="time_minutes">
                                    @for($j=0;$j<=3;$j++)
                                        <option value="{{ strlen($j*15) == 1 ? '00' : $j*15 }}"{{ Input::old('time.minutes', Date::parse($offer->time)->format('i')) == $j ? ' selected' : '' }}>{{ strlen($j*15) == 1 ? '00' : $j*15 }}</option>
                                    @endfor
                                </select>
                            </div>
                        </div>
